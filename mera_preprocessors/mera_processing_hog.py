...
def mera_circuit(inputs):
    ...
    output = [qml.expval(qml.PauliZ(i)) for i in range(NUM_QUBITS)]
    return output
...
def image_to_quantum(image, pca_model):
    ...
    output = mera_circuit(normalized)
...
def create_features(pca_model=None, split='Train'):
    ...
    with open(f'q_dataset/HOG_features/mera_features_{split}.pkl', 'wb') as f: pkl.dump(features, f)
    with open(f'q_dataset/HOG_features/mera_labels_{split}.pkl', 'wb') as f: pkl.dump(labels, f)
